...
(function (window) {
	 ...
	 */
	function autoInit() {
		var boxes = document.querySelectorAll('[data-inmemoriam-qr]'), i;

		for (i = 0; i < boxes.length; i++) {
			var box = boxes[i],
				done = render(box, box.getAttribute('data-inmemoriam-qr'), {
					level: 'Q',
	...
	/**
	 * Prepare l'impression d'une fiche.
	 *
	 * Masquer la page avec visibility:hidden ne suffit pas : les elements
	 * conservent la place qu'ils occupent, ce qui laisse de grands vides et
	 * repousse la fiche. On duplique donc la fiche en enfant direct de
	 * <body>, ce qui permet de masquer tout le reste sans se soucier de la
	 * profondeur a laquelle phpBB l'a imbriquee.
	 */
	function preparePrint() {
		var sheet = document.querySelector('.inmemoriam-sheet');

		if (!sheet) {
			return;
		}

		var old = document.querySelector('.inmemoriam-print');
		...
		var holder = document.createElement('div'),
			...
			canvas = sheet.querySelector('canvas'),
			target = clone.querySelector('[data-inmemoriam-qr]'),
			img, i;

		// Un canvas ne survit pas au clonage : on le remplace par une image.
		if (canvas && target) {
			try {
				...
				// Rien : le repli textuel du clone reste affiche.
			}
		}

		var hidden = clone.querySelectorAll('.inmemoriam-noprint');

		for (i = 0; i < hidden.length; i++) {
			hidden[i].parentNode.removeChild(hidden[i]);
		}

		holder.className = 'inmemoriam-print';
		holder.appendChild(clone);
		document.body.appendChild(holder);
	}

	function bindPrint() {
		var buttons = document.querySelectorAll('[data-inmemoriam-print]'), i;
		...
		// Certains navigateurs impriment sans passer par le bouton.
